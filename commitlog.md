...
## C08 — Remove unrelated whitespace from the HYPIR patch

- 动机：构建前的幂等补丁检查发现，末尾空格和换行的非功能差异使已修复节点不能通过反向检查。
- 范围：删除补丁中与错误处理无关的文件末尾格式修改，只保留模型加载及推理异常修复。
- 注意事项：不放宽补丁匹配条件，未知功能改动仍会阻止构建。
- 验证：当前节点反向 apply --check 通过；本地 HYPIR 两个工作流已完成实际推理，至此 18 项本地测试全部通过。

## C09 — Package ComfyUI as a configurable GPU container

- 动机：将已验证的本地环境封装为一个业务镜像，供 Ubuntu NVIDIA 机器通过配置和 Compose 启动，避免把大型模型或开发机环境复制进镜像。
- 范围：新增唯一 Dockerfile、构建忽略规则、离线 wheel 准备程序、构建/启动脚本、硬件配置、单服务 Compose 和部署说明；忽略构建缓存。
- 改造细节：固定 Python 基镜像 digest，从经过 RECORD 校验的 uv 缓存恢复 320 个依赖包，保留 wheel 元数据大小写；通过 BuildKit 只读挂载安装，修复 OpenCV/ONNX 的覆盖顺序并检查 CUDA 13.0。运行时显式绑定单张 GPU，模型只读挂载，用户、输入、输出及缓存分别持久化，增加健康检查。
- 配置边界：显卡编号、显存模式、预留显存、端口、路径和网络模式由 config.env 控制；更换 CUDA 版本需要重新准备和验证依赖，不能只改检查变量。默认 bridge 发布宿主端口，internal 模式用于 Linux 宿主直连容器 IP 的断网验收。
- 注意事项：构建机需保有部署用自定义节点源码及 uv 缓存；系统依赖准备需要联网，目标机使用 docker load 后无需联网构建。根 rail-system 编排已同步引用此 Dockerfile，但根目录不是 Git 仓库，其配置随交付目录保留。本次未启动其他业务服务。
- 验证：Dockerfile 静态检查、Shell 语法、Compose 配置通过；镜像构建及 GPU 健康检查成功。无外网默认路由的只读模型容器完成 18/18 个实际工作流，全部 node_errors 为空；检查镜像不含模型、宿主虚拟环境、wheel 目录或运行证书。最终端口、导出镜像和回读结果另记于验收报告。

## C10 — Record local and container deployment validation results

- 动机：为离线交付保留可审查的测试范围、实际结果和镜像来源，区分接口推理验证与画质、性能、整个平台验收。
- 范围：新增 deploy/VALIDATION.md，更新任务计划完成情况，记录每条工作流三轮耗时、模型与网络检查、已修复问题和复现命令。
- 交付记录：本地 HTTPS、断网容器、最终镜像发布端口各 18/18 通过，每轮下载 54 个输出；最终端口 WS 和基础 API 检查通过，所需 61 种节点齐全；最终镜像导出为 images/comfyui.tar，并保留 SHA-256、镜像 ID、来源提交和 docker load 回读记录。
- 注意事项：镜像代码对应 C09，C10 仅增加文档，未改变已验证运行代码。真实硬件为 3090，未声称完成 4090 或甲方双卡实测。小尺寸少步数为功能测试，原平台 8188 地址没有切换到容器；容器保持运行于本机 18188，恢复本地 HTTPS 的命令写入报告。
- 提交边界：模型实体、镜像 tar、原始运行证据及无关 Manager 缓存不加入 Git；根 rail-system 不是 Git 仓库，其整合配置与镜像清单随目录交付。
- 验证：三轮结果均检查 node_errors 为空、历史成功及实际输出；Git diff 格式检查通过。提交使用英文标题，按本文件 C01—C10 分别记录中文改动说明。
